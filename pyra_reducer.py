...
class PyraReducer:
    # is_rotation_move: Str -> Bool
    @staticmethod
    def is_rotation_move(move):
    ...
    @staticmethod
    def isolate_twist_elements(move):
    ...
    # isolate_rotation_elements: Str -> (Str, Str)
    @staticmethod
    def isolate_rotation_elements(rotation):
    ...
    # cancel_rotations: Iterable(Str) -> Iterable(Str)
    @staticmethod
    def cancel_rotations(moves):
        ...
        rotation_vertex_indices_map = {"L": [0, 2, 3], "R": [0, 3, 1], "U": [1, 3, 2], "B": [2, 0, 1]}
        ...
        face_indices_map = {"Lw": [0, 1, 3], "Rw": [0, 3, 2], "Dw": [1, 2, 3], "Fw": [2, 1, 0]}
        indices_face_map = {"013": "Lw", "023": "Rw", "123": "Dw", "012": "Fw"}

        for move in moves:
            if PyraReducer.is_rotation_move(move):
                rotation_axis, rotation_amount_str = PyraReducer.isolate_rotation_elements(move)
                ...
                for _ in range(rotation_amount):
                    ...
                    for j in range(len(vertices_to_swap) - 1, 0, -1):
                        virtual_pyra_corners[vertices_to_swap[j]] = virtual_pyra_corners[vertices_to_swap[j - 1]]
                    virtual_pyra_corners[vertices_to_swap[0]] = temp
            else:
                # $move is a face-turn move
                layers, face, rotation_amount_str = PyraReducer.isolate_twist_elements(move)
                ...
                vertices_on_face = [virtual_pyra_corners[index] for index in indices_for_turn]
